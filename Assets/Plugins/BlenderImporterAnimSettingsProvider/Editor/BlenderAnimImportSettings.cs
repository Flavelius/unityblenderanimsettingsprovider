...
namespace Client
{
    internal class BlenderAnimImportSettings : ScriptableObject
    {
        ...
        [Tooltip("Useful in case of shared projects, automatically applies the settings patch on load")]
        [SerializeField] bool autoApply;

        const string DefaultAssetPath = "Assets/Plugins/BlenderImporterAnimSettingsProvider/Editor/" + nameof(BlenderAnimImportSettings) + ".asset";
        ...
        void AutoApplyIfNecessary()
        {
            ...
            {
                Debug.LogError("Cannot auto-apply animation clip type patch to .blend importer, more than 1 settings file exists in project (potentially ambiguous value to apply), please make sure only one such file exists, found assets:");
            ...
            bool TryApplyPatch(string[] scriptLines, AnimationImportTypes replacedValue, out string errorText)
            {
                for (int i = 0; i < scriptLines.Length; i++)
                {
                    if (scriptLines[i].IndexOf(ImportKeyword, StringComparison.OrdinalIgnoreCase) >= 0)
                    {
                        ...
                        if (parts.Length != 2)
                        {
                            errorText = "Unexpected content in FBX importer script file";
                            return false;
                        }
                        var valueSet = false;
                        for (int j = 1; j < parts.Length; j++)
